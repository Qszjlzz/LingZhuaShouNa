...
export function MyPlansScreen({ onBack, nativeState, onNativeChange }: { onBack: () => void; nativeState?: NativeState | null; onNativeChange?: () => void }) {
  const [tab, setTab] = useState<(typeof tabs)[number]>("全部");
  const [openCalendar, setOpenCalendar] = useState(false);

  if (openCalendar) return <CalendarScreen nativeState={nativeState} onNativeChange={onNativeChange} onBack={() => setOpenCalendar(false)} />;
