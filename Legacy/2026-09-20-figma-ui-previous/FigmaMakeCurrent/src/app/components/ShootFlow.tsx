      ...
      >
        ...
        >
          <input
            ...
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void push(input); } }}
            placeholder="告诉我想怎么调整…"
            className="flex-1 bg-transparent outline-none"
            style={{ color: COFFEE, fontSize: 13, lineHeight: 1.5 }}
          />
          <button
            onClick={() => void push(input)}
            disabled={sending}
          ...
          >
            {sending ? <span style={{ color: WHITE, fontSize: 11 }}>...</span> : <Send size={15} color={WHITE} />}
          </button>
        </div>
    ...
    <div className="relative h-full w-full overflow-hidden" style={{ backgroundColor: "#1a1411" }}>
      ...
      >
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={() => on完成(shots)}
            disabled={shots.length === 0 || capturing}
        ...
        {nativeError && <p className="mt-2 text-center" style={{ color: "#C25C43", fontSize: 11 }}>{nativeError}</p>}
...
type DetectedItem = { id: string; name: string; confidence: number; emoji?: string; category?: string; suggestedZone?: string; isSelected?: boolean };
type BlindSpot = {
  id: string;
  label: string;
  reason: string;
    ...
    <div className="h-full w-full flex flex-col" style={{ backgroundColor: LINEN }}>
      ...
      >
        ...
        <button
          ...
          disabled={!allResolved || loadingItems || items.length === 0}
          className="w-full py-3.5 mt-5"
          style={{
            backgroundColor: allResolved && items.length > 0 ? ORANGE : SOFT,
            color: WHITE,
            borderRadius: 999,
            fontSize: 14,
            fontWeight: 600,
        ...
        >
          {loadingItems ? "正在读取识别结果…" : items.length === 0 ? "未识别到物品，请返回补拍" : allResolved ? "确认并继续" : `请先确认 ${blindSpots.filter((b) => !b.resolved).length} 个低置信度物品`}
        </button>
      </div>
    ...
    return (
      <div className="relative h-full w-full overflow-hidden" style={{ backgroundColor: "#1a1411" }}>
        ...
        <div className="absolute bottom-0 left-0 right-0 pb-10 pt-6 px-8 flex items-center justify-center">
          ...
          {captureError && <p className="absolute bottom-3 text-center" style={{ color: WHITE, fontSize: 11 }}>{captureError}</p>}
        </div>
      </div>
    );
  }
